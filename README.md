...
backend/
  ...
  requirements.txt
  requirements-dev.txt        + pytest/httpx for the test suite (not needed to run the app)
...
Full writeup, file-by-file, with a test-evidence appendix: **[SECURITY.md](SECURITY.md)**.
Short version, mapped to the challenge's evaluation rubric:

- **Data minimization**: raw webcam video never leaves the browser. Only
  calibration-fitted screen coordinates are used client-side; only
  aggregated behavioral events are sent to the backend — no biometric
  imagery is ever persisted or transmitted.
